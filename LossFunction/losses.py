...
def SoftmaxLoss():
    ...
    def softmax_loss(y_true, y_pred):
        ...
        # y_pred: logist
        print(f"BEFORE - y_true: {y_true.shape} - y_predict: {y_pred.shape}")
        y_true = tf.cast(tf.reshape(y_true, [-1]), tf.int32)
        print(f"AFTER  - y_true: {y_true.shape} - y_predict: {y_pred.shape}")
...
def CosfaceLoss(margin, scale, n_classes):
    def cosface_loss(y_true, y_pred):
        ...
        logist *= scale
        ce = tf.nn.softmax_cross_entropy_with_logits(labels=y_true, logits=logist)
        return tf.reduce_mean(ce)
